...
def is_img1(x):
    if x.endswith('.jpg') and not(x.startswith('._')):
        return True
    else:
        return False
...
class get_train(data.Dataset):
    ...
    def _scan(self):
        ...
        list_h = [os.path.splitext(x)[0]+'.jpg' for x in list_c]
...
#        xh = math.floor(img_h.shape[0]/32)*16
#        yh = math.floor(img_h.shape[1]/32)*16
...
#        xc = math.floor(img_c.shape[0]/32)*16
#        yc = math.floor(img_c.shape[1]/32)*16
...
class get_test(data.Dataset):
    ...
    def _load_file(self, idx):
        ...
        xh = math.floor(img_h.shape[0]/24)*16
        yh = math.floor(img_h.shape[1]/24)*16
        img_h = cv2.resize(img_h, (yh, xh), interpolation=cv2.INTER_CUBIC)
        
        img_c = cv2.cvtColor(cv2.imread(file_c), cv2.COLOR_BGR2RGB)
        if np.max(img_c)>1: img_c = img_c/255.0   # 归一化
        
        xc = math.floor(img_c.shape[0]/24)*16
        yc = math.floor(img_c.shape[1]/24)*16
        img_c = cv2.resize(img_c, (yc, xc), interpolation=cv2.INTER_CUBIC)   
