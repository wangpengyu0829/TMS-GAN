...
class get_train(data.Dataset):
    def __init__(self, real_path, haze_path, clean_path, patch_size):
        self.patch_size = patch_size   
        self.real_path  = real_path 
        self.haze_path  = haze_path 
        self.clean_path = clean_path 
        self._set_filesystem(self.real_path, self.haze_path, self.clean_path)   # 训练图像路径 / groudtruth 路径
        self.images_r, self.images_h, self.images_c = self._scan()             # 获得训练和 GT 图像
        self.repeat = 1
        
    '''打印路径'''        
    def _set_filesystem(self, dir_r, dir_h, dir_c):
        self.dir_r = dir_r
        self.dir_h = dir_h
        self.dir_c = dir_c
        print('********* Train dir *********')
    ...
    def _scan(self):
        ...
        list_r = [os.path.splitext(x)[0]+'.png' for x in list_c]
        list_r = [os.path.join(self.dir_r, os.path.split(x)[-1]) for x in list_r]  # 根据list_c中的图像名+.png 遍历训练图像路径中的图像，其名字形成列表
        random.shuffle(list_r)
        return list_r, list_h, list_c                  

    def __getitem__(self, idx):
        img_r, img_h, img_c, filename_r, filename_h, filename_c = self._load_file(idx)      # 获取图像
        ...
        img_r = img_r[x : x+self.patch_size, y : y+self.patch_size, :]
        img_h = _np2Tensor(img_h)                                        # 转化为 tensor
        img_c = _np2Tensor(img_c)
        img_r = _np2Tensor(img_r)
        return img_r, img_h, img_c
    ...
    def _load_file(self, idx):
        ...
        file_r = self.images_r[idx]   # 选取训练图像名
        ...
        img_r = cv2.cvtColor(cv2.imread(file_r),   cv2.COLOR_BGR2RGB)
        if np.max(img_r)>1: img_r = img_r/255.0    
        img_r = cv2.resize(img_r, (img_c.shape[1], img_c.shape[0]), interpolation=cv2.INTER_CUBIC)
        ...
        filename_r = os.path.splitext(os.path.split(file_r)[-1])[0]   # GT图像每个图像的图像名
        return img_r, img_h, img_c, filename_r, filename_h, filename_c                   # 输出图像和图像名
    
    
'''读取训练数据'''
class get_test(data.Dataset):
    def __init__(self, haze_path, clean_path):
        self.haze_path  = haze_path 
        self.clean_path = clean_path 
    ...
    def _load_file(self, idx):
        ...
        xh = math.floor(img_h.shape[0]/32)*32
        yh = math.floor(img_h.shape[1]/32)*32
        img_h = cv2.resize(img_h, (yh, xh), interpolation=cv2.INTER_CUBIC)
        
        img_c = cv2.cvtColor(cv2.imread(file_c), cv2.COLOR_BGR2RGB)
        if np.max(img_c)>1: img_c = img_c/255.0   # 归一化
        
        xc = math.floor(img_c.shape[0]/32)*32
        yc = math.floor(img_c.shape[1]/32)*32
